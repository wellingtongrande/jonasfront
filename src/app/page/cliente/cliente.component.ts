...
export class ClienteComponent implements OnInit {
    ...
    ordemColunasTabela = ['id', 'nome', 'idade', 'telefone', 'senha', 'email', 'excluir', 'editar'];
    totalElementos = 0;
    pagina = 0;
    tamanho = 5;
    ...
    submit() {
      ...
      if (formValues.id) {
        this.clienteService.update(cliente).subscribe(resposta => {
          this.listarClientes(this.pagina, this.tamanho);
    ...
    montarFormulario() {
      this.formulario = this.formBilder.group({
        ...
        telefone: [null, [Validators.minLength(1), Validators.maxLength(12)]],
        senha: [null, [Validators.minLength(3), Validators.maxLength(20)]],
        email: [null, [Validators.minLength(4), Validators.maxLength(30)]]
      })
    }
    ...
    listarClientes(pagina: number, tamanho: number) {// definir a primeira página e o tamanho inicial
    ...
    editar(id: number) {
      this.clienteService.findClienteById(id).subscribe((response) => {
        ...
        this.formulario.controls.name.setValue(response.nome);
